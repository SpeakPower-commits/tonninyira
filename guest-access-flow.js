...
(function(){
  ...
  /* roleChoice() used to fire on `role==='customer'` too -- but afterAuth()
     already stamps every new sign-in with role:'customer' immediately, so
     that condition meant EVERY established customer saw "How will you use
     Tonninyira?" again on every single checkout, not just once at signup.
     A profile row missing entirely (!p.data) is the only case that still
     genuinely has no role decision on file. */
  async function requireAuth(action){const s=await session();if(s){pendingTransaction=action;const p=await client().from('profiles').select('role').eq('id',s.user.id).maybeSingle();if(p.data&&(p.data.role==='admin'||p.data.role==='staff')){close();location.href='./admin-control-tower.html';return}if(!p.data)roleChoice();else action();return}pendingTransaction=action;authStart()}
